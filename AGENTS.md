...
| `KV_REST_API_URL` | Redis REST API URL. Falls back to in-memory if unset. Auto-set by Upstash KV integration on Vercel. |
...
- **Persistence**: `@vercel/kv` (Redis) when `KV_REST_API_URL` is set; in-memory fallback locally. Data is lost on server restart without KV.
